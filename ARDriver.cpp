...
{
	mbInitialised = true;
	mirFBSize.width = Persistence::PV3::get<int>("ARDriver.FrameBufferSize", 1200, Persistence::SILENT);
	mirFBSize.height = Persistence::PV3::get<int>("ARDriver.FrameBufferSize", 900, Persistence::SILENT);
...
void ARDriver::Render(cv::Mat &imFrame, RigidTransforms::SE3<> se3CfromW)
{
	if (!mbInitialised)
	{
		Init();
...
void ARDriver::MakeFrameBuffer()
{
	...
	std::cout << "  ARDriver: Creating FBO... ";
		...
		for (int ystep = 0; ystep < nStepsY; ystep++)
		{
			...
			for (int xstep = 0; xstep <= nStepsX; xstep++)
				for (int yystep = ystep; yystep <= ystep + 1; yystep++) // Two y-coords in one go - magic.
				{
					...
					cv::Vec2d v2UFBUnDistorted = mCamera.UFBLinearProject(mCamera.UFBUnProject(v2UFBDistorted));
	...
	if (bFirstRun)
	{
		...
		for (int ystep = 0; ystep < nStepsY; ystep++)
		{
			...
			for (int xstep = 0; xstep <= nStepsX; xstep++)
				for (int yystep = ystep; yystep <= ystep + 1; yystep++) // Two y-coords in one go - magic.
				{
					cv::Vec2d v2Iter((double)xstep / nStepsX, (double)yystep / nStepsY);
					cv::Vec2d v2UFBDistorted = v2Iter;
					cv::Vec2d v2UFBUnDistorted = mCamera.UFBLinearProject(mCamera.UFBUnProject(v2UFBDistorted));
