...
{
	mse3ViewerFromWorld =
		RigidTransforms::SE3<>::exp(cv::Vec<double, 6>(0, 0, 2, 0, 0, 0)) *
		RigidTransforms::SE3<>::exp(cv::Vec<double, 6>(0, 0, 0, 0.8 * CV_PI, 0, 0));
}
...
void MapViewer::DrawMapDots()
{
	...
	glPointSize(3);
	glBegin(GL_POINTS);
	mv3MassCenter = cv::Vec3d(0, 0, 0);
	for (size_t i = 0; i < mMap.vpPoints.size(); i++)
	{
		cv::Vec3d v3Pos = mMap.vpPoints[i]->v3WorldPos;
		glColor3d(gavLevelColors[mMap.vpPoints[i]->nSourceLevel][0],
			gavLevelColors[mMap.vpPoints[i]->nSourceLevel][1],
			gavLevelColors[mMap.vpPoints[i]->nSourceLevel][2]);
...
void MapViewer::DrawMap(RigidTransforms::SE3<> se3CamFromWorld)
{
	mMessageForUser.str(""); // Wipe the user message clean

	// Update viewer position according to mouse input:
	{
		std::pair<cv::Vec6d, cv::Vec6d > pv6 = mGLWindow.GetMousePoseUpdate();
		RigidTransforms::SE3<> se3CamFromMC;
		se3CamFromMC.get_translation() = mse3ViewerFromWorld * mv3MassCenter;
		mse3ViewerFromWorld = RigidTransforms::SE3<>::exp(pv6.first) *
			se3CamFromMC * RigidTransforms::SE3<>::exp(pv6.second) * se3CamFromMC.inverse() * mse3ViewerFromWorld;
	}
...
void MapViewer::SetupModelView(RigidTransforms::SE3<> se3WorldFromCurrent)
{
	glMatrixMode(GL_MODELVIEW);
	glLoadIdentity();
...
void MapViewer::DrawCamera(RigidTransforms::SE3<> se3CfromW, bool bSmall)
{
	...
	if (!bSmall)
	{
		...
		glColor3f(0.5, 0.5, 0.5);
		SetupModelView();
		cv::Vec3d se3Vec = se3CfromW.inverse().get_translation();
		cv::Vec2d v2CamPosXY(se3Vec[0], se3Vec[1]);
		glBegin(GL_LINES);
		glColor3f(1, 1, 1);
