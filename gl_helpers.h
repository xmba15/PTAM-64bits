...
/// Specify the (x,y) co-ordinates of a vertex
	...
	///@ingroup gGL
	inline void glMultiTexCoord(GLenum unit, const cv::Point& i)
	{
	...
	/// @param c The new colour
	///@ingroup gGL
	inline void glColor(const cv::Vec3f& c)
	{
	...
	/// @param c The new colour
	///@ingroup gGL
	inline void glColor3(const cv::Vec4b& c)
	{
		glColor3ub(c(2), c(1), c(0));
	}
	...
	/// @param c The new colour
	///@ingroup gGL
	inline void glColor4(const cv::Vec4b& c)
	{
	...
	/// @param c The new colour
	///@ingroup gGL
	inline void glColor(const cv::Vec4f& c)
	{
		glColor4f(c(2), c(1), c(0), c(3));
	}
	...
	///@ingroup gGL
	inline void glDrawPixels(const cv::Mat& i)
	{
	...
	///@ingroup gGL
	inline void glReadPixels(cv::Mat &i, cv::Point origin = cv::Point(0,0))
	{
		::glReadPixels(origin.x, origin.y, i.cols, i.rows, GL_RGB, GL_UNSIGNED_BYTE, i.data);
	}
	...
	/// @param origin The window co-ordinate of the first pixel to be read from the frame buffer
	///@ingroup gGL
	inline cv::Mat glReadPixels(cv::Size size, cv::Point origin = cv::Point(0, 0))
	{
		cv::Mat i(size.height, size.width, CV_8UC3);
		::glReadPixels(origin.x, origin.y, i.size().width, i.size().height, GL_RGB, GL_UNSIGNED_BYTE, i.data);
		return i;
	}
