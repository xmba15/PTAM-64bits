...
{
	bool bFixed;
	RigidTransforms::SE3<> se3CfW;
	RigidTransforms::SE3<> se3CfWNew;
	cv::Matx<double, 6, 6> m6U;          // Accumulator
	cv::Vec<double, 6> v6EpsilonA;   // Accumulator
	int nStartRow;
};
...
struct BAPoint
{
	...
	cv::Vec3d v3Pos;
	cv::Vec3d v3PosNew;
	cv::Matx<double, 3, 3> m3V;          // Accumulator
	cv::Vec3d v3EpsilonB;   // Accumulator 
	cv::Matx<double, 3, 3> m3VStarInv;
...
struct Meas
{
  ...
  cv::Matx<double, 2,3> m23B;
  cv::Matx<double, 6,3> m63W; 
  cv::Matx<double, 6,3> m63Y;
...
public:
  ...
  int AddCamera(RigidTransforms::SE3<> se3CamFromWorld, bool bFixed); // Add a viewpoint. bFixed signifies that this one is not to be adjusted.
  int AddPoint(cv::Vec3d v3Pos);       // Add a map point.
  void AddMeas(int nCam, int nPoint, cv::Vec2d v2Pos, double dSigmaSquared); // Add a measurement
  ...
  cv::Vec3d GetPoint(int n);       // Point coords after adjustment
  RigidTransforms::SE3<> GetCamera(int n);            // Camera pose after adjustment
