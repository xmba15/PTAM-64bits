...
namespace glExceptions
{
	...
	public:
	    ...
	    virtual void on_resize(GLWindow& /*win*/, cv::Point /*size*/) {}
	...
	GLWindow(const cv::Size& size, int bpp=24, const std::string& title="GLWindow") {
	  init(size, bpp, title);
	}
	GLWindow(const cv::Size& size, const std::string& title, int bpp=24) {
	  init(size, bpp, title);
	}
    ...
    private:
	State* state;
	void init(const cv::Size& size, int bpp, const std::string& title);
    };
#endif
