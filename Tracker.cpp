...
Tracker::Tracker(cv::Size irVideoSize, const ATANCamera &c, Map &m, MapMaker &mm) :
	mMap(m),
	mMapMaker(mm),
	mCamera(c),
	...
	mirSize(irVideoSize)
{
	...
	Persistence::GUI.RegisterCommand("Reset", GUICommandCallBack, this);
	Persistence::GUI.RegisterCommand("KeyPress", GUICommandCallBack, this);
	Persistence::GUI.RegisterCommand("PokeTracker", GUICommandCallBack, this);
...
void Tracker::Reset()
{
	...
	mnFrame = 0;
	mv6CameraVelocity = cv::Vec6d::all(0);
...
void Tracker::TrackFrame(cv::Mat_<uchar> &imFrame, bool bDraw, cv::Mat &rgbFrame)
{
	...
	static Persistence::pvar3<double> gvdSBIBlur("Tracker.RotationEstimatorBlur", 0.75, Persistence::SILENT);
	static Persistence::pvar3<int> gvnUseSBI("Tracker.UseRotationEstimator", 1, Persistence::SILENT);
	...
	if (mbDraw)
	{
		...
		if (Persistence::PV3::get<int>("Tracker.DrawFASTCorners", 0, Persistence::SILENT))
		{
			glColor3f(1, 0, 1);  glPointSize(1); glBegin(GL_POINTS);
			for (unsigned int i = 0; i < mCurrentKF->aLevels[0].vCorners.size(); i++)
				GLXInterface::glVertex(mCurrentKF->aLevels[0].vCorners[i]);
			glEnd();
		}
	}
...
bool Tracker::AttemptRecovery()
{
	...
	RigidTransforms::SE3<> se3Best = mRelocaliser.BestPose();
...
void Tracker::RenderGrid()
{
	...
	cv::Mat_<cv::Vec2d>  imVertices(nTot, nTot);
	for (int i = 0; i < nTot; i++)
		for (int j = 0; j < nTot; j++)
		{
			cv::Vec3d v3((i - nHalfCells) * 0.1,
				         (j - nHalfCells) * 0.1,
				         0.0);
			cv::Vec3d v3Cam = mse3CamFromWorld * v3;
			if (v3Cam[2] < 0.001)
				v3Cam[2] = 0.001;
	...
	for (int i = 0; i < nTot; i++)
	{
		...
		for (int j = 0; j < nTot; j++)
			glVertex2d(imVertices(i, j)[0], imVertices(i, j)[1]);
		glEnd();

		glBegin(GL_LINE_STRIP);
		for (int j = 0; j < nTot; j++)
			glVertex2d(imVertices(j, i)[0], imVertices(j, i)[1]);
		glEnd();
	}
...
void Tracker::GUICommandHandler(string sCommand, string sParams)  // Called by the callback func..
{
	...
	if (sCommand == "KeyPress")
	{
		...
		{
			Persistence::GUI.ParseLine("quit");
		}
		return;
	}
...
void Tracker::TrackForInitialMap()
{
	...
	static Persistence::pvar3<int> gvnMaxSSD("Tracker.MiniPatchMaxSSD", 100000, Persistence::SILENT);
	...
	if (mnInitialStage == TRAIL_TRACKING_STARTED)
	{
		...
		{
			mbUserPressedSpacebar = false;
			std::vector<std::pair<cv::Point, cv::Point> > vMatches;   // This is the format the mapmaker wants for the stereo pairs
			for (std::list<Trail>::iterator i = mlTrails.begin(); i != mlTrails.end(); i++)
				vMatches.push_back(std::pair<cv::Point, cv::Point>(i->irInitialPos,
					i->irCurrentPos));
...
bool cornerCompare(const std::pair<double, cv::Point> &left, const std::pair<double, cv::Point> &right) {
...
void Tracker::TrailTracking_Start()
{
	...
	mFirstKF = mCurrentKF;
	std::vector<std::pair<double, cv::Point> > vCornersAndSTScores;
	...
	{                                                                     // so that we can choose the image corners with max ST score
		...
		if (!CvUtils::in_image_with_border(c.irLevelPos.y,
			...
			continue;
		vCornersAndSTScores.push_back(std::pair<double, cv::Point>(-c.dSTScore, c.irLevelPos)); // negative so highest score first in sorted list
	}

	std::sort(vCornersAndSTScores.begin(), vCornersAndSTScores.end(), cornerCompare);  // Sort according to Shi-Tomasi score
	int nToAdd = Persistence::PV3::get<int>("MaxInitialTrails", 1000, Persistence::SILENT);
...
int Tracker::TrailTracking_Advance()
{
	...
	for (std::list<Trail>::iterator i = mlTrails.begin(); i != mlTrails.end();)
	{
		...
		Trail &trail = *i;
		cv::Point irStart = trail.irCurrentPos;
		cv::Point irEnd = irStart;
		...
		if (bFound)
		{
			...
			cv::Point irBackWardsFound = irEnd;
			...
			if ((irBackWardsFound.x - irStart.x) * (irBackWardsFound.x - irStart.x) +
				(irBackWardsFound.y - irStart.y) * (irBackWardsFound.y - irStart.y) > 2)
				bFound = false;
...
void Tracker::TrackMap()
{
  ...
  std::vector<TrackerData::Ptr> avPVS[LEVELS];
  for(int i=0; i<LEVELS; i++)
    avPVS[i].reserve(500);
  ...
  for (unsigned int i = 0; i < mMap.vpPoints.size(); i++)
  {
	  ...
	  if (!p->pTData) p->pTData.reset(new TrackerData(p));
	  TrackerData::Ptr TData = p->pTData;
  ...
  std::vector<TrackerData::Ptr> vNextToSearch;
  std::vector<TrackerData::Ptr> vIterationSet;
  
  // Tunable parameters to do with the coarse tracking stage:
  static Persistence::pvar3<unsigned int> gvnCoarseMin("Tracker.CoarseMin", 20, Persistence::SILENT);   // Min number of large-scale features for coarse stage
  static Persistence::pvar3<unsigned int> gvnCoarseMax("Tracker.CoarseMax", 60, Persistence::SILENT);   // Max number of large-scale features for coarse stage
  static Persistence::pvar3<unsigned int> gvnCoarseRange("Tracker.CoarseRange", 30, Persistence::SILENT);       // Pixel search radius for coarse features
  static Persistence::pvar3<int> gvnCoarseSubPixIts("Tracker.CoarseSubPixIts", 8, Persistence::SILENT); // Max sub-pixel iterations for coarse features
  static Persistence::pvar3<int> gvnCoarseDisabled("Tracker.DisableCoarse", 0, Persistence::SILENT);    // Set this to 1 to disable coarse stage (except after recovery)
  static Persistence::pvar3<double> gvdCoarseMinVel("Tracker.CoarseMinVelocity", 0.006, Persistence::SILENT);  // Speed above which coarse stage is used.
      ...
      if(vNextToSearch.size() < nCoarseMax)
	{
	  ...
	  if (nFound >= *gvnCoarseMin)  // Were enough found to do any meaningful optimisation?
	  {
		  ...
		  for (int iter = 0; iter < 10; iter++) // If so: do ten Gauss-Newton pose updates iterations.
		  {
			  ...
			  cv::Vec6d v6Update = CalcPoseUpdate(vIterationSet, dOverrideSigma);
...
int Tracker::SearchForPoints(std::vector<TrackerData::Ptr> &vTD, int nRange, int nSubPixIts)
{
	int nFound = 0;
	for (unsigned int i = 0; i < vTD.size(); i++)   // for each point..
	{
		...
		bool bFound =
			Finder.FindPatchCoarse(TD->v2Image, mCurrentKF, nRange);
		TD->bSearched = true;
		if (!bFound)
		{
			TD->bFound = false;
...
cv::Vec<double, 6> Tracker::CalcPoseUpdate(std::vector<TrackerData::Ptr> vTD, double dOverrideSigma, bool bMarkOutliers)
{
	// Which M-estimator are we using?
	int nEstimator = 0;
	static Persistence::pvar3<string> gvsEstimator("TrackerMEstimator", "Tukey", Persistence::SILENT);
	...
	cv::Matx<double, 6, 6> m6Omega = 100.0 * cv::Matx<double, 6, 6>::eye();
	cv::Vec<double, 6> v6ksi(0, 0, 0, 0, 0, 0);
	...
	for (unsigned int f = 0; f < vTD.size(); f++)
	{
		...
		if (!TD->bFound)
			continue;
		cv::Vec2d &v2 = TD->v2Error_CovScaled;
		double dErrorSq = v2[0] * v2[0] + v2[1] * v2[1];
		double dWeight;
		...
		m6Omega += dWeight * (TD->dSqrtInvNoise * TD->dSqrtInvNoise) * (TD->m26Jacobian.t() * TD->m26Jacobian);
		v6ksi += dWeight * ((TD->dSqrtInvNoise * TD->m26Jacobian.t()) * v2);
	}
...
void Tracker::ApplyMotionModel()
{
	...
	cv::Vec6d v6Velocity = mv6CameraVelocity;
	if (mbUseSBIInit)
	{
	...
	mse3CamFromWorld = RigidTransforms::SE3<>::exp(v6Velocity) * mse3StartPos;
}
...
{
	RigidTransforms::SE3<> se3NewFromOld = mse3CamFromWorld * mse3StartPos.inverse();
	cv::Vec6d v6Motion = RigidTransforms::SE3<>::ln(se3NewFromOld);
	cv::Vec6d v6OldVel = mv6CameraVelocity;
	...
	cv::Vec6d v6 = mv6CameraVelocity;
	double tempVal = 1.0 / mCurrentKF->dSceneDepthMean;
	v6[0] *= tempVal;
	v6[1] *= tempVal;
	v6[2] *= tempVal;
...
void Tracker::AssessTrackingQuality()
{
	...
	else
	{
		...
		static Persistence::pvar3<double> gvdQualityGood("Tracker.TrackingQualityGood", 0.3, Persistence::SILENT);
		static Persistence::pvar3<double> gvdQualityLost("Tracker.TrackingQualityLost", 0.13, Persistence::SILENT);
...
void Tracker::CalcSBIRotation()
{
	...
	RigidTransforms::SE3<> se3Adjust = SmallBlurryImage::SE3fromSE2(result_pair.first, mCamera);
	mv6SBIRot = se3Adjust.ln();
}

cv::Size TrackerData::irImageSize;  // Static member of TrackerData lives here
